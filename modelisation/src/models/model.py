...
#     data = pd.read_csv(data_path, sep=";", parse_dates=['JOUR'], index_col='JOUR')
#     models = auto_arima(data, seasonal=True, m=7)  # m est la période saisonnière
...
def find_and_save_complete_name(data_path, column_name):
    """
    On sauvegarde la liste des catégories uniques qui sont présentes pour chaque jour dans nos données.
    Cela nous permet de ne pas gérer des situations avec des données manquantes.
    """
    data = pd.read_csv(data_path, sep=";", parse_dates=['JOUR'], index_col='JOUR')
    data = data[data.index <= "2019-12-01"]
    save_path = 'models/'+column_name+'.npy'
...
def model_total_validation(data_path, is_model_load=True, plot=False):
    ...
    # future_predictions = models.predict(future_data)
